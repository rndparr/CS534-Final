...
```{r}
dat <- read.csv("/Users/Courtney/Documents/Emory_3rd_Semester/CS - Machine Learning/CS534-Final/data/Hospital_Inpatient_Discharges__SPARCS_De-Identified___2015.csv")
...
# Subset data 
dat <- dat[!(dat$APR.MDC.Code == 14 | dat$Abortion.Edit.Indicator == "Y" | dat$Type.of.Admission == "Newborn" | dat$APR.MDC.Code == 15),]
```
...
```{r}
ggplot(aes(x = APR.Severity.of.Illness.Description, y = Length.of.Stay), data = dat) + stat_summary(fun = "mean", geom = "bar") +
...
```{r}
ggplot(aes(x = Type.of.Admission, y = Cost.per.Day), data = dat) + stat_summary(fun = "mean", geom = "bar",  fill = "#69b3a2") +
...
```{r}
ggplot(aes(x = APR.Severity.of.Illness.Description, y = Cost.per.Day), data = dat) + stat_summary(fun = "mean", geom = "bar",  fill = "#69b3a2") +
...
```{r}
ggplot(aes(x = Age.Group, y = Cost.per.Day), data = dat) + stat_summary(fun = "mean", geom = "bar",  fill = "#69b3a2") +
