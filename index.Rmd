...
library(lme4)
library(spaMM)
spaMM.options('nb_cores' = future::availableCores())
```
...
# If Facility.Type and Ownership.Type are NA, then make it it's own category of Missing
# dat$Facility.Type = factor(dat$Facility.Type, levels=c(levels(dat$Facility.Type), 'Missing'))
# dat[which(is.na(dat$Facility.Type)), 'Facility.Type'] <- 'Missing'
dat$Facility.Type = factor(dat$Facility.Type, exclude = NULL)

# dat$Ownership.Type = factor(dat$Ownership.Type, levels=c(levels(dat$Ownership.Type), 'Missing'))
# dat[which(is.na(dat$Ownership.Type)), 'Ownership.Type'] <- 'Missing'
dat$Ownership.Type = factor(dat$Ownership.Type, exclude = NULL)
...
<!-- We found that 100% of missing data for Facility.Id, Health Service Area, and Hospital County also had Abortion.Edit.Indicator = 'Y', meaning the discharge record had some mention of abortion.  -->
...
The $R^2$ value for the simple linear regression on the training data is `{r, cache = TRUE} round(summary(linearReg)$r.squared, 4)`. 
...
The output shows a $R^2$ of `{r, cache = TRUE} #round(summary(linearReg_Cost)$r.squared, 4)` on the training data.
