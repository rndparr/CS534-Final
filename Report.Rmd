...
dat$Type.of.Admission <- factor(dat$Type.of.Admission,
  labels = c('Elective', 'Trauma', 'Emergency', 'Urgent', 'Newborn', 'Not Available'))
...
#   width = '800px', 
#   height = '400px',
#   extra_css = 'position: center; display: block; margin: auto;')
```
...
# filter_obgyn(aprdrg$APR.DRG.Description)
# aprdrg_obgyn_desc <- grep(obgyn_terms, aprdrg$APR.DRG.Description, ignore.case = TRUE, value = TRUE)
# aprdrg_obgyn_desc <- grep(exclude_terms, aprdrg_obgyn_desc, ignore.case = TRUE, value = TRUE, invert = TRUE)
...
# ccs_obgyn_desc <- grep(exclude_terms, ccs_obgyn_desc, ignore.case = TRUE, value = TRUE, invert = TRUE)
...
```{r, los-by-aprseverity-medicare, class.source = NULL, message = FALSE}
...
```{r, los-by-age-medicare, class.source = NULL, message = FALSE}
...
evaluate_model <- function(model, test, response){
  ...
  return(c('Mean Absolute Error' = MAE(pred, actual), 
    'Mean Relative Error' = MRE(pred, actual), 
    'Root Mean Squared Error' = RMSE(pred, actual)))
}
```
...
```{r, cpd-hlin-reg-eval, eval = FALSE}
# Evaluation metrics
