...
<!-- dat_hf <- read.csv('input/Health_Facility_General_Information.csv',
  colClasses =  c('factor', 'NULL', rep('factor', 2), rep('NULL', 32)),
  skipNul = TRUE)
colnames(dat_hf) <- c('Facility.Id', 'Facility.Type.Short', 'Facility.Type')

unique(dat_hf[, c('Facility.Type.Short', 'Facility.Type')])


bleh <- apply(dat_hf[, c('Facility.Type.Short', 'Facility.Type')], 1, paste0, collapse = '_')
unique(bleh)
unique(dat_hf[, c('Facility.Type.Short', 'Facility.Type')])
 -->

[@newyorkstatedepartmentofhealthHealthFacilityGeneral2014]

```{r, data-import-healthfacility}
# read in facility id and description columns from facility information dataset
dat_hf <- read.csv('input/Health_Facility_General_Information.csv',
  colClasses =  c('factor', rep('NULL', 2), 'factor', rep('NULL', 32)),
  skipNul = TRUE)
colnames(dat_hf) <- c('Facility.Id', 'Facility.Type')

# merge datasets by Facility Id
dat <- merge(dat, dat_hf, by = 'Facility.Id')
```
...
medicare_rows <- with(dat, Vectorize(isTRUE)(Payment.Typology.1 == 'Medicare' | Payment.Typology.2 == 'Medicare' | Payment.Typology.3 == 'Medicare'))
...
evaluate_model <- function(model, test, response){
  ...
  return(c('Mean Absolute Error' = MAE(pred, actual), 
    'Mean Relative Error' = MRE(pred_mre, actual_mre), 
